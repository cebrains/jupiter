	...
	for mod, ns := range nodes {
		...
		for _, n := range ns {
			...
			data[mod][n.Level] = append(data[mod][n.Level], map[string]interface{}{
				...
				"position": "https://github.com/cebrains/jupiter/blob/master/" + filepath + "#L" + strconv.Itoa(n.Position.Line),
			})
		}
	}
	...
	for _, desc := range configDescs {
		...
		if _, ok := configMap[desc.Package]; ok {
			...
			if desc.Name != "" {
				...
				mdData["position"] = "https://github.com/cebrains/jupiter/blob/master/" + filepath + "#L" + strconv.Itoa(desc.Position.Line)
			}
